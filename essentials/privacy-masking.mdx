...
By default, all input values are masked. Configure per input type:

```typescript
sessionRecordingMasking: {
  maskAllInputs: true,  // Master switch
  maskInputOptions: {
...
3. **Enable automatic masking** for inputs by default (`maskAllInputs: true`)
