...
Signal JS is a comprehensive SDK for session replay, network monitoring, and analytics - inspired by PostHog-JS. Record user sessions, capture network requests, monitor console logs, and track user behavior with built-in privacy protection.
...
|-----------|---------|-------------|
| **Vanilla JS / HTML** | `signal-js` | `npm install signal-js` |
| **Vue.js** | `signal-js` | `npm install signal-js` |
...
> **Note:** 
> - Framework packages (`@signal-js/react`, `@signal-js/nextjs`) automatically include the browser SDK (`signal-js`). You only need to install one package.
