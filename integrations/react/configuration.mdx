...
Configure the React provider and the underlying browser SDK via `SignalProvider`'s `options` prop. Required: `endpoint`, `apiKey`, `projectId`.
...
`options` is passed to `createSignal`. Required: `endpoint`, `apiKey`, `projectId`.
...
- **urlDenyList** — Array of URL strings or RegExp. Requests whose URL matches are **excluded** from capture (no URL, headers, or body stored). Use to skip health checks, third-party analytics, or any URL that must not appear in replays.
