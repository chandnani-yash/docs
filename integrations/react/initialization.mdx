...
Wrap your app with `SignalProvider` at the root (e.g. in `main.tsx` or `App.tsx`). Pass `options` with your endpoint, API key, and project ID.
...
|------|------|---------|-------------|
| `options` | `SignalOptions` | required | Browser SDK config. Required: `endpoint`, `apiKey`, `projectId`. See [Configuration](/docs/integrations/react/configuration). |
