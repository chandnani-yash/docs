...
`createSignal(options)` accepts the browser SDK options. Required: `endpoint`, `apiKey`, `projectId`. Same options apply in Vue, Vanilla JS, Svelte, Angular, or any framework.
...
- **urlDenyList** — Array of URL strings or RegExp. Requests whose URL matches are **excluded** from capture. Use to skip health checks, analytics, or any URL that must not appear in replays.
